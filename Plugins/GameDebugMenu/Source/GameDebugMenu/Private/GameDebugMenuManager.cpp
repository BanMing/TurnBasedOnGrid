...
void AGameDebugMenuManager::BeginPlay()
{
	...
	GetWorld()->GetTimerManager().SetTimer(InitializeManagerHandle, FTimerDelegate::CreateLambda([this]() { OnInitializeManager(); }), 0.01f, true);
}
...
EGDMPropertyType AGameDebugMenuManager::GetPropertyType(const FProperty* TargetProperty) const
{
	if (TargetProperty != nullptr)
	{
		...
		else if (TargetProperty->IsA(FStructProperty::StaticClass()))
		{
			...
			else if (StructProp->Struct->GetFName() == NAME_IntPoint)
			{
				return EGDMPropertyType::GDM_IntPoint;
			}
...
bool AGameDebugMenuManager::RegisterObjectProperty(UObject* TargetObject, const FName PropertyName, const FGDMGameplayCategoryKey& CategoryKey, const FString& PropertySaveKey, const FText& DisplayPropertyName,
	const FText& Description, const FGDMPropertyUIConfigInfo& PropertyUIConfigInfo, const int32& DisplayPriority)
{
	...
	ObjectProperties.Sort([](const TSharedPtr<FGDMObjectPropertyInfo>& A, const TSharedPtr<FGDMObjectPropertyInfo>& B) { return A->DisplayPriority >= B->DisplayPriority; });
	...
	if (!PropertySaveKey.IsEmpty())
	{
		...
		switch (PropertyType)
		{
			case EGDMPropertyType::GDM_Bool:
			{
				SET_VARIANT_PROPERTY(FBoolProperty) break;
			}
			case EGDMPropertyType::GDM_Int:
			{
				SET_VARIANT_PROPERTY(FIntProperty) break;
			}
			case EGDMPropertyType::GDM_Float:
			{
				SET_VARIANT_PROPERTY(FFloatProperty) break;
			}
			case EGDMPropertyType::GDM_Enum:
			{
				SET_VARIANT_ENUM_PROPERTY() break;
			}
			case EGDMPropertyType::GDM_Byte:
			{
				SET_VARIANT_PROPERTY(FByteProperty) break;
			}
			case EGDMPropertyType::GDM_String:
			{
				SET_VARIANT_PROPERTY(FStrProperty) break;
			}
			case EGDMPropertyType::GDM_Vector:
			{
				SET_VARIANT_STRUCT_PROPERTY(FVector) break;
			}
			case EGDMPropertyType::GDM_Vector2D:
			{
				SET_VARIANT_STRUCT_PROPERTY(FVector2D) break;
			}
			case EGDMPropertyType::GDM_IntPoint:
			{
				SET_VARIANT_STRUCT_PROPERTY(FIntPoint) break;
			}
			case EGDMPropertyType::GDM_Rotator:
			{
				SET_VARIANT_STRUCT_PROPERTY(FRotator) break;
			}
			default:
		...
		else
		{
			switch (PropertyType)
			{
				case EGDMPropertyType::GDM_Bool:
				{
					CALL_PROPERTY_DISPATCHER(Bool, FBoolProperty, bool) break;
				}
				case EGDMPropertyType::GDM_Int:
				{
					CALL_PROPERTY_DISPATCHER(Int, FIntProperty, int32) break;
				}
				case EGDMPropertyType::GDM_Float:
				{
					CALL_PROPERTY_DISPATCHER(Float, FFloatProperty, float) break;
				}
				case EGDMPropertyType::GDM_Enum:
				{
					CALL_ENUM_PROPERTY_DISPATCHER() break;
				}
				case EGDMPropertyType::GDM_Byte:
				{
					CALL_PROPERTY_DISPATCHER(Byte, FByteProperty, uint8) break;
				}
				case EGDMPropertyType::GDM_String:
				{
					CALL_PROPERTY_DISPATCHER(String, FStrProperty, FString) break;
				}
				case EGDMPropertyType::GDM_Vector:
				{
					CALL_STRUCT_PROPERTY_DISPATCHER(Vector, FVector);
					break;
				}
				case EGDMPropertyType::GDM_Vector2D:
				{
					CALL_STRUCT_PROPERTY_DISPATCHER(Vector2D, FVector2D);
					break;
				}
				case EGDMPropertyType::GDM_IntPoint:
				{
					CALL_STRUCT_PROPERTY_DISPATCHER(IntPoint, FIntPoint);
					break;
				}
				case EGDMPropertyType::GDM_Rotator:
				{
					CALL_STRUCT_PROPERTY_DISPATCHER(Rotator, FRotator);
					break;
				}
				default:
					UE_LOG(LogGDM, Warning, TEXT("Unsupported property type for old value caching"));
...
bool AGameDebugMenuManager::RegisterObjectFunction(
	UObject* TargetObject, const FName FunctionName, const FGDMGameplayCategoryKey& CategoryKey, const FString& FunctionSaveKey, const FText& DisplayFunctionName, const FText& Description, const int32& DisplayPriority)
{
	if (!IsValid(TargetObject))
	{
	...
	ObjectFunctions.Sort([](const TSharedPtr<FGDMObjectFunctionInfo>& A, const TSharedPtr<FGDMObjectFunctionInfo>& B) { return A->DisplayPriority >= B->DisplayPriority; });
...
UObject* AGameDebugMenuManager::GetObjectProperty(const int32 Index, FGDMGameplayCategoryKey& OutCategoryKey, FString& OutPropertySaveKey, FText& OutDisplayPropertyName, FText& OutDescription, FName& OutPropertyName,
...
UObject* AGameDebugMenuManager::TryGetObjectProperty(const FString& InPropertySaveKey, const FString& InPropertyName, FGDMGameplayCategoryKey& OutCategoryKey, FText& OutDisplayPropertyName, FText& OutDescription,
...
void AGameDebugMenuManager::CallChangePropertyIntPointDispatcher(const FName& PropertyName, UObject* PropertyOwnerObject, FIntPoint New, FIntPoint Old, const FString& PropertySaveKey)
{
	TArray<UGDMListenerComponent*> ListenerComponents;
	UGDMListenerComponent::GetAllListenerComponents(GetWorld(), ListenerComponents);
	...
	for (const auto& Component : ListenerComponents)
	{
		Component->OnChangePropertyIntPointDispatcher.Broadcast(PropertyName, PropertyOwnerObject, New, Old, PropertySaveKey);
	}
}
